//! Paint a [`bliss_dom::BaseDocument`] by pushing [`anyrender`] drawing commands into
...
use bliss_dom::{BaseDocument, util::Color};
use render::BlissDomPainter;
...
/// Paint a [`bliss_dom::BaseDocument`] by pushing drawing commands into
/// an impl [`anyrender::PaintScene`].
///
...
pub fn paint_scene(
    ...
    let generator = BlissDomPainter::new(
        doc,
        scale,
        width,
        height,
...
fn process_custom_widget_node(
    ...
    use bliss_dom::node::{CustomWidgetStatus, ProxyRenderContext};
