...
#[repr(u8)]
pub enum UiEvent {
    PointerMove(BlissPointerEvent),
    PointerUp(BlissPointerEvent),
    PointerDown(BlissPointerEvent),
    Wheel(BlissWheelEvent),
    KeyUp(BlissKeyEvent),
...
pub enum DomEventData {
    ...
    PointerDown(BlissPointerEvent),
    PointerUp(BlissPointerEvent),
    PointerEnter(BlissPointerEvent),
    PointerLeave(BlissPointerEvent),
    PointerOver(BlissPointerEvent),
    PointerOut(BlissPointerEvent),

    MouseMove(BlissPointerEvent),
    MouseDown(BlissPointerEvent),
    MouseUp(BlissPointerEvent),
    MouseEnter(BlissPointerEvent),
    MouseLeave(BlissPointerEvent),
    MouseOver(BlissPointerEvent),
    MouseOut(BlissPointerEvent),

    Scroll(BlissScrollEvent),
    Wheel(BlissWheelEvent),

    Click(BlissPointerEvent),
    ContextMenu(BlissPointerEvent),
    DoubleClick(BlissPointerEvent),

    KeyPress(BlissKeyEvent),
    KeyDown(BlissKeyEvent),
    KeyUp(BlissKeyEvent),
    Input(BlissInputEvent),
    Ime(BlissImeEvent),

    Focus(BlissFocusEvent),
...
#[derive(Clone, Debug)]
pub struct BlissPointerEvent {
    pub id: BlissPointerId,
    pub is_primary: bool,
...
impl BlissPointerEvent {
    #[inline(always)]
    pub fn is_mouse(&self) -> bool {
        matches!(self.id, BlissPointerId::Mouse)
    }
    #[inline(always)]
    pub fn is_pen(&self) -> bool {
        matches!(self.id, BlissPointerId::Pen)
    }
    #[inline(always)]
    pub fn is_finger(&self) -> bool {
        matches!(self.id, BlissPointerId::Finger(_))
    }
...
#[derive(Clone, Debug)]
pub enum BlissWheelDelta {
    Lines(f64, f64),
    Pixels(f64, f64),
}

#[derive(Clone, Debug)]
pub struct BlissWheelEvent {
    pub delta: BlissWheelDelta,
    pub coords: PointerCoords,
    pub buttons: MouseEventButtons,
    pub mods: Modifiers,
}

impl BlissWheelEvent {
    #[inline(always)]
    pub fn page_x(&self) -> f32 {
        self.coords.page_x
    }
