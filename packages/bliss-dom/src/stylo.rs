...
///
/// Since BlissNodes are not persistent (IE we don't keep the pointers around between frames), we choose to just implement
/// the tree structure in the nodes themselves, and temporarily give out pointers during the layout phase.
type BlissNode<'a> = &'a Node;

impl<'a> TDocument for BlissNode<'a> {
    type ConcreteNode = BlissNode<'a>;
...
impl<'a> TShadowRoot for BlissNode<'a> {
    type ConcreteNode = BlissNode<'a>;
...
impl<'a> TNode for BlissNode<'a> {
    type ConcreteElement = BlissNode<'a>;
    type ConcreteDocument = BlissNode<'a>;
    type ConcreteShadowRoot = BlissNode<'a>;
...
impl selectors::Element for BlissNode<'_> {
...
impl<'a> TElement for BlissNode<'a> {
    type ConcreteNode = BlissNode<'a>;
...
impl std::hash::Hash for BlissNode<'_> {
...
fn assert_size_of_equals() {
    ...
    // let size = mem::size_of::<StyleSharingCandidate<BlissNode>>();
    // dbg!(size);
}
