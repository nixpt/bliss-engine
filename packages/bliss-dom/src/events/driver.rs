...
use bliss_traits::events::{
    BlissPointerEvent, BlissPointerId, DomEvent, DomEventData, EventPhase, EventSink, EventState,
    UiEvent,
};
...
impl<'doc, Handler: EventHandler> EventDriver<'doc, Handler> {
    ...
    pub fn handle_pointer_move(&mut self, event: &BlissPointerEvent) -> Option<usize> {
    ...
    pub fn handle_ui_event(&mut self, event: UiEvent) {
        ...
        match &event {
            ...
            UiEvent::PointerUp(event) => {
                ...
                if event.is_primary && matches!(event.id, BlissPointerId::Finger(_)) {
    ...
    fn handle_pointer_event(
        &mut self,
        target: usize,
        data: BlissPointerEvent,
        make_ptr_data: impl FnOnce(BlissPointerEvent) -> DomEventData,
        make_mouse_data: impl FnOnce(BlissPointerEvent) -> DomEventData,
    ) {
